...
import presto.simple_roots as sr
...
class dedisp_method(object):
    def __init__(
        ...
        if numsub:  # Calculate the maximum subband smearing we can handle
            ...
            while 1:
                ...
                next_ss = subband_smear(
                    next_dsubDM, numsub, self.obs.BW, self.obs.f_ctr
                )
                ...
                if next_ss > 0.8 * min(self.BW_smearing, 1000.0 * obs.dt * downsamp):
                    ...
                    self.sub_smearing = subband_smear(
                        self.dsubDM, numsub, self.obs.BW, self.obs.f_ctr
                    )
                    break
                DMs_per_prepsub += 2
        else:
            ...
            if numsub:
                ...
                if numprocs > 1 and self.numprepsub % numprocs:
                    ...
                    while (
                        ...
                        and self.numprepsub * self.DMs_per_prepsub > self.numDMs
                    ):
                        self.DMs_per_prepsub -= 1
    ...
    def DM_for_newparams(self, dDM, downsamp):
        ...
        return (
            0.001 * other_smear / self.obs.chanwidth * 0.0001205 * self.obs.f_ctr**3.0
        )
    ...
    def plot(self, work_fract):
        ...
        ppgplot.pgptxt(
            midDM,
            np.log10(1.1 * self.total_smear(midDM)),
            90.0,
            0.0,
        ...
        if self.numsub:
            ...
            ppgplot.pgptxt(
                midDM,
                np.log10(0.85 * self.sub_smearing),
                0.0,
                0.5,
    ...
    def transientx_format(self, snrloss=0.1, maxwidth=0.02):
        """
        Return the parameters in TransientX ddplan format:
        td fd dms ddm ndm snrloss maxwidth(s)
        """
        return "%d 1 %.3f %.2f %d %.1f %.2f" % (
            self.downsamp,  # td
            self.loDM,      # dms  
            self.dDM,       # ddm
            self.numDMs,    # ndm
...
def dm_smear(DM, BW, f_ctr, cDM=0.0):
    ...
    """
    return 1000.0 * pu.dm_smear(np.fabs(DM - cDM), BW, f_ctr)
...
def guess_DMstep_approx(dt, BW, f_ctr):
    """
...
def guess_DMstep(dt, BW, f_ctr):
    ...
    def func(x, BW=BW, f_ctr=f_ctr):
        return pu.dm_smear(x, BW, f_ctr) - dt

    return sr.bisect(func, 0.0, 1000.0) * 2.0
...
def dm_steps(
    ...
    min_chan_smearing = dm_smear(
        np.linspace(loDM, hiDM, 10000), obs.chanwidth, obs.f_ctr, cohdm
    ).min()
    ...
    for ii, offset in enumerate(np.add.accumulate([0] + numDMs[:-1])):
        ...
        total_smears[offset : offset + numDMs[ii]] = methods[ii].total_smear(
            methods[ii].DMs
        )
    ...
    if device is not None:
        ...
        pgp.plotxy(
            ...
            labx="Dispersion Measure (pc/cm\\u3\\d)",
            laby="Smearing (ms)",
            device=device,
        )
        ppgplot.pgsch(1.1)
        ...
        if numsub:
            ppgplot.pgmtxt(
                "t", 1.5, 0.6 / 10.0, 0.5, "\\(2156)\\dctr\\u = %g MHz" % obs.f_ctr
            )
            if dtms < 0.1:
                ppgplot.pgmtxt(
                    "t", 1.5, 2.8 / 10.0, 0.5, "dt = %g \\gms" % (dtms * 1000)
                )
            else:
            ...
            ppgplot.pgmtxt(
                "t", 1.5, 1.0 / 8.0, 0.5, "\\(2156)\\dctr\\u = %g MHz" % obs.f_ctr
            )
            if dtms < 0.1:
                ppgplot.pgmtxt(
                    "t", 1.5, 3.0 / 8.0, 0.5, "dt = %g \\gms" % (dtms * 1000)
                )
            else:
        ...
        if numsub:
            ppgplot.pgsci(12)
            ppgplot.pgmtxt(
                "b", 1 * dy, 0.97, 1.0, "Subband Stepsize Smearing (# passes)"
            )
        ppgplot.pgsci(11)
...
def usage():
  ...
  [-x, --transientx]              : Output TransientX ddplan format
...
"""
                ...
                if "CHAN_DM" in pf.header:
                    ...
                    if cDM != 0.0:
                        print(
                            "And assuming channels coherently dedispersed at DM = %g pc/cm^3"
                            % cDM
                        )
            except:
    ...
    for o, a in opts:
        ...
        if o in ("-o", "--output"):
            ...
            if not (
                device.endswith("/CPS")
                or device.endswith("/VCPS")
                or device.endswith("/cps")
                or device.endswith("/vcps")
            ):
                device += "/CPS"
        if o in ("-l", "--loDM"):
        ...
        if o in ("-r", "--res"):
            ok_smearing = float(a)
        if o in ("-x", "--transientx"):
            transientx_output = True
    ...
    methods = dm_steps(
        loDM, hiDM, obs, cDM, numsubbands, numprocs, ok_smearing, blocklen, device
    )

    # TransientX output format
    if transientx_output:
        print("\n# TransientX ddplan format:")
        print("# td fd dms ddm ndm snrloss maxwidth(s)")
        for method in methods:
            print(method.transientx_format())
